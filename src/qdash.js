...
const qd = {
  ...
  begin: function () {
    ...
    var basesize =
      (qd.config.width / 100) * (30 / n) +
      "vw"; /* to be used as base value for all elements */
    ...
    for (var p = 0; p < pList.length; p++) {
      ...
      for (var i = 0; i < eList.length; i++) {
        ...
        if (elem.hasAttribute("entity")) {
          ...
          elem.innerHTML =
            // temporary set content to detect mistakes
  ...
  showSection: function (sections) {
    ...
    for (var i = 0; i < sList.length; i++) {
      if (v[0] == sList[i])
        document.querySelector(`[section="${sList[i]}"]`).style.display =
          "block";
      else
        document.querySelector(`[section="${sList[i]}"]`).style.display =
          "none";
  ...
  requestNextBox: function (data) {
    ...
    if (data) {
      ...
      if (data.key != qd.config.key) {
        qd.log(
          `requestNextBox - key mismatch: ix=${data.ix} key=${qd.config.key} err=${data.key}`
        );
    ...
    if (ix < qd.box.length) {
      ...
      mqtt.send(
        qd.config.requestTopic,
        `{"ent":"${ent}","ix":"${ix}","key":"${qd.config.key}"}`
      );
  ...
  configBox: function (data) {
    ...
    function setBoxLookup(src) {
      ...
      for (var k = 0; k < pairs.length; k++) {
        ...
        else if (p.length == 1) box.lookup[p[0]] = p[0]; //val,val,val,...
      }
    }
    ...
    try {
      // call custom handler
      if (typeof qd.config.onConfigBox === "function")
        qd.config.onConfigBox(data);
      var ent = data.ent;
      ...
      var list = getAttr("list", data.list);
      setBoxLookup(list);
      var color = getAttr("color", data.color);
      setBoxLookup(color);
      var dict = getAttr("dict", data.dict);
      setBoxLookup(dict);
      ...
      switch (domain) {
        ...
        case "input_boolean":
          ...
          if (show) html += `<span val ${style}></span>`;
            ...
            html += `<span val onclick="qd.operation( '${ent}','${box.set}','' )" ${style}></span>`; // on or off
          else
            html += `<span val onclick="qd.operation( '${ent}','toggle','' )" ${style}></span>`;
          break;
        ...
        case "script":
          ...
          if (show) html += `<span val ${style}></span>`;
            ...
            html += `<span val onclick="qd.operation( '${ent}','run','${box.set}' )" ${style}></span>`;
          break;
        ...
        case "light":
          ...
          if (show) html += `<span val ${style}></span>`;
            ...
            html += `<span val onclick="qd.operation( '${ent}','turn${box.set}','' )" ${style}></span>`; // turnon or turnoff
          else
            html += `<span val onclick="qd.operation( '${ent}','toggle','' )" ${style}></span>`;
          break;
        ...
        case "sensor":
          if (box.prec || box.uom) {
            // this is a numeric sensor
            if (color) box.update = "numeric_icon";
            else box.update = "state_numeric";
          } // this is a text sensor
          else if (dict) box.update = "state_text";
          else if (color) box.update = "state_icon";
        ...
        case "input_number":
          if (show) {
            if (color) box.update = "numeric_icon";
            else if (dict) box.update = "state_lookup";
            else box.update = "state_numeric";
            html += `<span val ${style}></span>`;
            break;
          }
          ...
          if (range) {
            ...
            var d = range.split(",");
            if (d.length != 3)
              html += `<input val type="number" min="0" max="100" step="1" `;
            else
              html += `<input val type="number" min="${d[0]}" max="${d[1]}" step="${d[2]}" `;
  ...
  updateBox: function (data) {
    ...
    try {
      if (typeof qd.config.onUpdateBox === "function")
        qd.config.onUpdateBox(data);
      var ent = data.ent;
      ...
      for (var i = 0; i < eList.length; i++) {
        ...
        if (!ve) {
          qd.debug("not yet configured: " + ent);
          return;
        ...
        switch (box.update) {
          ...
          case "onoff_icon": { // show state as colored on/off icon
            //
            ve.innerHTML = `<span class=" mdi mdi-${
              box.icon
            }" style="color:${lookup()}" ></span>`;
            break;
          }
          ...
          case "state_set": // show state==x as colored on/off icon
            var col =
              data.state == box.set ? box.lookup["on"] : box.lookup["off"];
            ve.innerHTML = `<span class=" mdi mdi-${box.icon}" style="color:${col}"></span>`;
            break;
          ...
          case "bright": // show light:bright
            if (data.state == "off") value = "0";
              ...
              value =
                com.round(
                  ((parseFloat(data.bright) * 10) / 255).toFixed(2),
                  "0"
                ) + "0";
